...
def run(i3d, frequency, frames_dir, batch_size, sample_mode, last_segment):
    ...
    for batch_id in range(batch_num):
        batch_data = load_rgb_batch(frames_dir, rgb_files, frame_indices[batch_id])  # [20,16,256,340,3]=b,freq,h,w,c
        if (sample_mode == 'oversample'):
            batch_data_ten_crop = oversample_data(batch_data)  # {list: 10}, each shape = [20,16,256,340,3]
            for i in range(10):
                assert (batch_data_ten_crop[i].shape[-2] == 224)
                assert (batch_data_ten_crop[i].shape[-3] == 224)
                temp = forward_batch(batch_data_ten_crop[i])  # [20,2048, 1,1 1]
    ...
    full_features = [np.concatenate(i, axis=0) for i in full_features]  # {list: 10, 57}, each shape = [20,2048, 1,1 1], 20 is batch size, 57 is no. of batches
    full_features = [np.expand_dims(i, axis=0) for i in full_features]  # {list: 10}, each shape = [1140,2048, 1,1,1] -> {list: 10}, each shape = [1,1140,2048, 1,1,1]
